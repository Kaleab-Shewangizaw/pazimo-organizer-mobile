...
import type { EventOutstandingBeverageItem } from "@/types";

/**
 * An event sells only drinks at this counter — no seats, no ticket types —
 * so like VenueScanner there is one kind of code (the bare payment
 * reference) and one kind of item, no picker. An event's pickup code is a
 * plain CODE128 barcode over BeverageSale.referenceNumber (see
 * backend/src/utils/barcodeRenderer.js), not a JSON QR payload, so whatever
 * the scanner decodes IS the reference to look up, with no parsing step.
 *
 * The event-scoped twin of VenueScanner — the one real difference is the
 * 12-hour post-event expiry gate (a venue never closes; an event does), so
 * this also has to render an "Expired" state the venue counter never needs.
 */
type Pending = {
  reference: string;
  items: EventOutstandingBeverageItem[];
  isExpired: boolean;
} | null;
...
/**
 * Camera-driven barcode scanner for one event's beverage counter — look an
 * order up by its pickup code, then hand its drinks over one at a time. The
 * camera only mounts while this screen is actually focused, matching
 * CinemaScanner/VenueScanner: only one CameraView may be active at a time.
 */
export function EventBeverageScanner({
  eventTitle,
  onChangeEvent,
}: {
  eventTitle: string;
  ...
  const lookup = async (reference: string) => {
    ...
    setLooking(true);
    try {
      const response = await getOutstandingEventBeverages(reference);
      ...
      setPending({ reference, items, isExpired: response.isExpired });
    } catch (error) {
      setOutcome({
        kind: "error",
        title: "Not found",
  ...
  const collect = async (item: EventOutstandingBeverageItem) => {
    setCollectingId(item._id);
    setCollectError(null);
    try {
      await redeemEventBeverage(item._id);
      setPending((prev) => {
        if (!prev) return prev;
      ...
      <View style={styles.overlay} pointerEvents="box-none">
        ...
        >
          <LinearGradient colors={CHROME_GRADIENT} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.topBar}>
            ...
            <Pressable onPress={onChangeEvent} style={styles.changeButton}>
          ...
          <View style={styles.bottomStack}>
            {pending ? (
              ...
              <HintBubble>{looking ? "Checking code…" : "Point the camera at a pickup barcode"}</HintBubble>
            )}
          </View>
        </SafeAreaView>
      </View>
...
function PendingCard({
  ...
  onCollect: (item: EventOutstandingBeverageItem) => void;
  onCancel: () => void;
}) {
  return (
        ...
        {pending.isExpired ? (
          <View style={styles.expiredBanner}>
            ...
            <Text style={styles.expiredBannerText}>
              This event ended more than 12 hours ago — nothing here can be collected
            </Text>
          </View>
        ) : null}
          ...
          {pending.items.map((item) => (
            <View key={item._id} style={styles.itemRow}>
              ...
              <Pressable
                ...
                disabled={pending.isExpired || collectingId === item._id}
                style={[styles.owedButton, pending.isExpired && styles.owedButtonDisabled]}
              >
